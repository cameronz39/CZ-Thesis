...
void rt_OneStep(void)
{
   ...
   */
  motorPositionTest_SetEventsForThisBaseStep(eventFlags);
    ...
    switch (2)
    {
     case 2 :
      currentTime = (extmodeSimulationTime_T)
        ((motorPositionTest_M->Timing.clockTick2 * 500) + 0)
        ;
      motorPositionTest_step2();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(motorPositionTest_M, 0);
  ...
  motorPositionTest_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(motorPositionTest_M->extModeInfo,
    (extmodeSimulationTime_T *)rteiGetPtrTFinalTicks
    (motorPositionTest_M->extModeInfo));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(motorPositionTest_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(motorPositionTest_M);
